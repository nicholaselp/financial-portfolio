...
@Transactional
@Rollback
public class FinancialPortfolioApiControllerDelegateTest extends SpringTests{
    ...
    public void create_expense_api_call() throws Exception {
        ...
        var response = extractResponse(result, ExpenseEntityDto.class);

        assertThat(response.getExpense().getExpense()).isEqualTo(expenseDto.getExpense());
    }
    ...
    public void update_expense_api_call() throws Exception {
        ...
        assertThat(expenseRepository.findById(storedExpense.getId())).isPresent()
                .hasValueSatisfying(exp -> {
                    assertThat(exp.getExpense()).isEqualTo("rent");
                });

        var updateExpenseDto = ExpenseTestFactory.createExpenseDto();
        updateExpenseDto.setExpense("updatedExpense");
        ...
        assertThat(response.getExpense().getExpense()).isEqualTo(updateExpenseDto.getExpense());
    }
    ...
    public void get_expense_by_id_api_call() throws Exception {
        ...
        assertThat(response.getExpense().getExpense()).isEqualTo(storedExpense.getExpense());
    }
